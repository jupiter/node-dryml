...
var render = exports.render = function(str, options, callback) {
    ...
    if (options.filename && !options.debug && options.cache === true) {
        path = realPath(options.filename, options.root);
...
function compileTag(forTaglib, tag, options) {
    if (tag.children) {
        var source = toFunctionSource(tag.children, 'definition', options),
            fn;
        try {
            fn = new Function('context, attributes, taglib, buffer, callback, _renderFunctions, sup', source);
        } catch(err) {
            console.log(source);
            throw err;
        }
...
function importTaglib(filename, options, callback) {
    ...
    if (!compiled || options.debug || options.cache === false) {
                    ...
                    case 'def':
                        if (element.attrs.tag) {
                            if (isValidTagname(element.attrs.tag)) {
                                ...
                                message = 'Invalid tag name\n    at <def tag="' + element.attrs.tag + '"> (' + filename + element.line + ':' + element.column + ')';
                                throw Error(message);
                            }
                        }
                        break;
...
function mergeIntoTaglib(taglib, newTaglib, options) {
