    ...
    for (var i in structure) {
        ...
        default:
            ...
            case 'with':
                ...
                tmp.push('(function(){');    
                var tagbodyVar = 'var tagbody = function(){ with(_argumentsAsObject(arguments, unescape("' + escape(attrs) + '"))) {' + toFunctionSource(tag.children, 'with', options) + '} };';
                if (ejs) {
                    if (async) {
                        tmp.push('buffer.print(buffer.async(this, function(buffer){ ' + tagbodyVar + ejs + '}));')                    
                    } else {
                        tmp.push(tagbodyVar);
                        tmp.push('tagbody.call(' + ejs + ');');
                    }                    
                } else {        
                    tmp.push(tagbodyVar);
                    if (attr) {
                        tmp.push('tagbody.call(unescape("' + escape(attr) + '"));');                                                  
                    } else {
                        tmp.push('tagbody();');                                                  
                    }   
                }
