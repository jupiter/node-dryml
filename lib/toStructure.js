 ...
 * @api private
 */
var EXTRACT_EJS_SOURCE = new RegExp(/(\<\%)([^\x00]*?)(?=\%\>)/g),
    EXTRACT_EJS_ATTR_SOURCE = new RegExp(/(\"%\{)(.*?)(\}\")|(\'%\{)(.*?)(\}\')/g);
function prepareString(str) {
    str = str.replace(EXTRACT_EJS_SOURCE, function(x, y, z){
        return y + escape(z);
    });

    str = str.replace(EXTRACT_EJS_ATTR_SOURCE, function(w, x, y, z, a, b, c, d, e, f){
        var opening = x || a || d,
            body = y || b || e,
            closing = z || c || f;
