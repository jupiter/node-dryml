...
var Buffer = function(callback) {
    var self = {
        ...
        print: function(str, middleware) {
            if (str !== null) {
                self.strArray.push((middleware) ? middleware(str) : str);
            }
        },
        async: function(scope, callback, trace) {
            ...
            var asyncBuffer = buffer[index] = {
                ...
                print: function(str, middleware) {
                    if (str !== null) {                    
                        asyncBuffer.strArray.push((middleware) ? middleware(str) : str);
                    }
                },
                end: function() {
                    asyncBuffer.str = asyncBuffer.strArray.join('');
            ...
            if (asyncBuffer.ended) {
              ...
              return asyncBuffer.str;
            } else {
              return replacementStr;
            }            
        },
        end: function() {
            self.str = self.strArray.join('');
            if (self.count === 0) {
